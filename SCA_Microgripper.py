# -*- coding: utf-8 -*-
"""
Created on Mon Sep 21 12:45:52 2020
...
def update_parameters(y):
    ...
    ##########################################################################################################################################################
    #Calculating Element Matrix and different boundary conditions
    ...
    # with np.printoptions(threshold=np.inf):
        #     print(n13)    
        ######################################################################################################################3
        #creating tags for different materials
    ...
    print("Max temp in polymer is %f K" %(Thatmax))
    ...
    ########################################################################################################################################################################################333
    #system matrices for mechanical part
    #########################################################################################################################################################
    ...
    Xx=np.zeros((Muu.shape[0], 1), dtype=float)
    ...
    X2=np.zeros((nnodes,1), dtype=float)
    ...
    print("Max displacement at X2[89] in polymer is %f um" %(X2[89]))
    print("Min displacement at X2[21] in polymer is %f um" %(X2[21]))
    # import matplotlib.pyplot as plt 
    # plt.plot(X2)
    
    return X2[21], Thatmax
...
def funcObjective(individual, type):
    ...
    elif type == 2:# Rosenbrock -30 .. 30
        print(individual)
        for x in range(0,len(individual)):
            min_disp, max_temp = update_parameters(individual[x])
            print(min_disp)
            fitness = min_disp + 0.0001*(max_temp-413)
    elif type == 3:
        # Rastrigin -5.12 .. 5.12
        for x in range(0,len(individual)):
...
searchAgents = 10
func = 2
lowerBound = np.array([0.00025, 12.0, 0.000005])
upperBound = np.array([0.00030, 14.0, 0.00001])
dist = 3
runs = 1
...
for z in range(0,runs):
    ...
    for x in range(0,searchAgents):
        ...
        for y in range(0,dim):
            ...
            # Gaussiana
                ...
                num = np.random.normal((lowerBound[y]+upperBound[y])/2, upperBound[y]-((lowerBound[y]+upperBound[y])/2))
                if num < lowerBound[y]:
                    num = lowerBound[y]
                if num > upperBound[y]:
                    num = upperBound[y]
    ...
    for x in range(0,searchAgents):
        min_disp, max_temp = update_parameters(population[x])
        fitness = min_disp + 0.0001*(max_temp-413)
        mean = mean + fitness
        if x == 0:
        ...
        for x in range(0,searchAgents):
                for y in range(0,dim):
                    ...
                    if r4 < 0.5:
                        ...
                        if population[x][y] > upperBound[y] or population[x][y] < lowerBound[y]:
                            # Uniforme
                            if dist == 1:
