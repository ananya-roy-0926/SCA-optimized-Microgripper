# -*- coding: utf-8 -*-
"""
Created on Wed Sep  9 21:48:26 2020

@author: Dell
"""


def update_parameters(L, width,J):
    ...
    #define points 
    gmsh.model.geo.addPoint(0, (0.000029-width-0.000020), 0, 0.00001, 1) 
    gmsh.model.geo.addPoint(0, (0.000029-width), 0, 0.00001, 2) 
    gmsh.model.geo.addPoint(0.000020, (0.000029-width), 0, 0.00001, 3)
    gmsh.model.geo.addPoint((0.000020+L)-0.000007, (0.000029-width), 0, 0.00001, 4) 
    ...
    gmsh.model.geo.addPoint((0.000020+L), (0.000029-width), 0, 0.00001, 13) 
    gmsh.model.geo.addPoint((0.000020+L), (0.000029-width-0.000022), 0, 0.00001, 14) 
    gmsh.model.geo.addPoint(0.000060, (0.000029-width-0.000022), 0, 0.00001, 15) 
    gmsh.model.geo.addPoint(0.000060, (0.000029-width-0.000006), 0, 0.00001, 16) 
    gmsh.model.geo.addPoint(0.000020, (0.000029-width-0.000006), 0, 0.00001, 17) 
    gmsh.model.geo.addPoint(0.000020, (0.000029-width-0.000020), 0, 0.00001, 18) 
    ...
    gmsh.model.geo.addPoint(0.000060, (0.000029-width), 0, 0.00001, 24) 
    ...
    #define points
    gmsh.model.geo.addPoint(0, (0.000095+width), 0, 0.00001, 175) 
    gmsh.model.geo.addPoint(0, (0.000075+width), 0, 0.00001, 176) 
    gmsh.model.geo.addPoint(0.000020, (0.000075+width), 0, 0.00001, 177) 
    gmsh.model.geo.addPoint((0.000020+L)-0.000007, (0.000075+width), 0, 0.00001, 178) 
    ...
    gmsh.model.geo.addPoint((0.000020+L), (0.000075+width), 0, 0.00001, 187) 
    gmsh.model.geo.addPoint((0.000020+L), (0.000097+width), 0, 0.00001, 188) 
    gmsh.model.geo.addPoint(0.000060, (0.000097+width), 0, 0.00001, 189) 
    gmsh.model.geo.addPoint(0.000060, (0.000081+width), 0, 0.00001, 190) 
    gmsh.model.geo.addPoint(0.000020, (0.000081+width), 0, 0.00001, 191) 
    gmsh.model.geo.addPoint(0.000020, (0.000095+width), 0, 0.00001, 192)
    ...
    gmsh.model.geo.addPoint(0.000060, (0.000075+width), 0, 0.00001, 198) 
    ...
    B=np.empty(shape=((len(B0)),6), dtype = np.int32)
    ...
    n1=np.empty(shape=((len(B2)),3), dtype = np.int32)
    ...
    n2=np.empty(shape=((len(B3)),3), dtype = np.int32)
    ...
    n3=np.empty(shape=((len(B4)),3), dtype = np.int32)
    ...
    n4=np.empty(shape=((len(B5)),3), dtype = np.int32)
    ...
    n5=np.empty(shape=((len(B6)),4), dtype = np.int32)
    ...
    n6=np.empty(shape=((len(B7)),4), dtype = np.int32)
    ...
    n7=np.empty(shape=((len(B8)),4), dtype = np.int32)
    ...
    n8=np.empty(shape=((len(B9)),4), dtype = np.int32)
    ...
    n9=np.empty(shape=((len(B10)),4), dtype = np.int32)
    ...
    n10=np.empty(shape=((len(B11)),4), dtype = np.int32)
    ...
    n11=np.empty(shape=((len(B12)),4), dtype = np.int32)
    ...
    n12=np.empty(shape=((len(B13)),4), dtype = np.int32)
    ...
    n13=np.empty(shape=((len(B)),4), dtype = np.int32)
    ...
    K = np.zeros((nnodes+1, nnodes+1), dtype=np.float32)  #Inisializing K, Stiffness matix
    C=np.zeros((nnodes+1, nnodes+1), dtype=np.float32)  #Inisializing C, Thermal matix

    
    #xcen = np.zeros(1,nels) ; ycen = xcen ; 

    x=np.zeros((nnodes+1,1), np.float32)
    y=np.zeros((nnodes+1,1), np.float32)
    z=np.zeros((nnodes+1,1), np.float32)
    ...
    ey=np.zeros((1,4), np.float32)
    ez=np.zeros((1,4), np.float32)
    ...
    for e in range(nels):
        ...
        bet=np.zeros((1,4), np.float32); 
        gam = np.zeros((1,4), np.float32); 
        delt=np.zeros((1,4), np.float32);
        ...
        Ke=np.empty((4,4),dtype=np.float32)
        ec=np.empty((4,4),dtype=np.float32)
    ...
    iT=293
    td=(np.ones((Aud.shape[1],1), dtype=np.float32))*iT;      
    ...
    tu=np.zeros((Auu.shape[1],1), dtype=np.float32)

    T1=np.zeros((Auu.shape[1],tf), dtype=np.float32)
    ...
    # f=np.zeros((Auu.shape[1],tf), dtype=np.float32)
    ...
    T2=np.zeros((nnodes,1), dtype=np.float32)
    ...
    That=np.zeros((nnodes,1), dtype=np.float32)
    ...
    T=np.zeros((nnodes,1), dtype=np.float32)
    ...
    E1=np.zeros((nnodes,1), dtype=np.float32)
    alp1=np.zeros((nnodes,1), dtype=np.float32)
    poisson1=np.zeros((nnodes,1), dtype=np.float32)
    sigma_th=np.zeros((nnodes,1), dtype=np.float32)
    ...
    K11=np.zeros((nnodes+1, nnodes+1), dtype=np.float32)  #Inisializing K, Stiffness matix
    K12=np.zeros((nnodes+1, nnodes+1), dtype=np.float32)
    K13=np.zeros((nnodes+1, nnodes+1), dtype=np.float32)
    K22=np.zeros((nnodes+1, nnodes+1), dtype=np.float32)
    K23=np.zeros((nnodes+1, nnodes+1), dtype=np.float32)
    K33=np.zeros((nnodes+1, nnodes+1), dtype=np.float32)

    Fd=np.zeros((nnodes+1,1), dtype=np.float32);

    Mass=np.zeros((nnodes+1, nnodes+1), dtype=np.float32)
    ...
    ex1=np.zeros((1,4), np.float32)
    ey1=np.zeros((1,4), np.float32)
    ez1=np.zeros((1,4), np.float32)
    ...
    for e in range(nels):
        ...
        bet1=np.zeros((1,4), np.float32); 
        gam1 = np.zeros((1,4), np.float32); 
        delt1=np.zeros((1,4), np.float32);
        ...
        ek=np.empty((4,4),dtype=np.float32)
        ...
        em=np.empty((4,4),dtype=np.float32)
        ...
        em=np.empty((4,1),dtype=np.float32)
    ...
    Zn=np.zeros((Muu1.shape[0],Muu1.shape[1]), dtype=np.float32)
    ...
    Zi=np.zeros((Muu1.shape[0],1), dtype=np.float32)
    Fhat=np.append(Zi, Fh11, axis=0)

################################################################################
    U=np.zeros((Mhat.shape[0],1), dtype=np.float32)

    U1=np.zeros((Mhat.shape[0],tf1), dtype=np.float32)
    ...
    # print("Min displacement at X2[21] in polymer is %f" %(X2[21]))
